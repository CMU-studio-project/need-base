...
class SentimentRoberta(ModelBase):
    def __init__(
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained(pretrained_model).to(
            self.device
        )
        self.model.eval()
    ...
    def predict_label(self, encoded_text: Dict[str, Any]) -> Tuple[str, float]:
        encoded_text = {key: val.to(self.device) for key, val in encoded_text.items()}
